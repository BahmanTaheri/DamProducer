...
namespace DamProducer
{
    public partial class frmResid : Form
    {
        ...
        private void frmResid_Load(object sender, EventArgs e)
        {
            ...
            this.tbl_NoeResidTA.FillByKharid(this.db_DataSetResid.Tbl_NoeResid);
        ...
        private bool NotError()
        {
            ...
            if (txtDateResid.Text.Contains("_"))
            {
            ...
            if ((string.IsNullOrEmpty(txtCodeFak.Text)) || (txtDateFak.Text.Contains("_")))
            {
                err.SetError(txtCodeFak, "تاریخ فاکتور را وارد  کنید");
                txtCodeFak.Focus();
                return false;
            }
            ...
            {
                err.SetError(txtNumCar, "شماره ماشین را وارد  کنید");
                txtNumCar.Focus();
                return false;
            }
        ...
        private void ubtnComite_Click(object sender, EventArgs e)
        {
            if (NotError())
            {
                ...
                UGrid.ActiveCell = UGrid.Rows[UGrid.ActiveRow.Index].Cells["Code_matter"];
        ...
        private void cmbFroshande_KeyUp(object sender, KeyEventArgs e)
        {
            ...
            if ((e.KeyData == Keys.Insert) && (function.InputBox("ورود", "نام فروشنده جدید را وارد کنید", ref strCode, function.DialogType.General, 0) == DialogResult.OK))
            {
                ...
                if (!string.IsNullOrEmpty(strCode))
                {
                    ...
                    try
                    {
                        foundIndex = viewPersonBS.Find("Name_fro", strCode);
                    }
                    catch
                    {
                        foundIndex = -1;
                    ...
                    if (foundIndex == -1)
                    {
                        this.view_PersonTA.InsertQuery(strCode);
                        this.view_PersonTA.Fill(this.db_DataSetResid.View_Person);
                    }
                }
            }
        ...
        private void txtHaml_Leave(object sender, EventArgs e)
        {
            ...
            UGrid_AfterCellUpdate(null, null);
        }
        ...
        private void printToolStripButton_Click(object sender, EventArgs e)
        {
            ...
            rpt1.Load(Application.StartupPath + @"\Report\rptResid.frx");
        ...
        private void UGrid_Enter(object sender, EventArgs e)
        {
        ...
        private void frmResid_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void UGrid_AfterCellUpdate(object sender, CellEventArgs e)
        {
            ...
            try
            {
                hazine = txtBargiri.IntValue + txtHaml.IntValue + txtOther.IntValue + txtTakhlie.IntValue; //- txtTakhfif.IntValue;
                Jamvazn = decimal.Parse(UGrid.Rows.SummaryValues["SumVazn"].Value.ToString());
            }
            catch { }
            ...
            try
            {
                txtBargiriK.Text = Math.Round(txtBargiri.IntValue / Jamvazn).ToString();
                txtHamlK.Text = Math.Round(txtHaml.IntValue / Jamvazn).ToString();
                txtTakhlieK.Text = Math.Round(txtTakhlie.IntValue / Jamvazn).ToString();
                txtOtherK.Text = Math.Round(txtOther.IntValue / Jamvazn).ToString();
                foreach (Infragistics.Win.UltraWinGrid.UltraGridRow row in UGrid.Rows)
                {
                    decimal tx = (decimal.Parse(row.Cells["Tax"].Text) - decimal.Parse(row.Cells["Takhfif"].Text)) / decimal.Parse(row.Cells["vazn"].Text);
                    hk = Math.Round(decimal.Parse(row.Cells["Price_vahed"].Text) + tx + hz, MidpointRounding.AwayFromZero);
